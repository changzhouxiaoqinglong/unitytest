...
namespace CodeAnimo.SurfaceWaves{
	...
	public class TerrainHeightRenderer : SimulationOutput {
		...
		private void SetupDepthClearMaterial(){
			...
			m_depthClearMaterial.SetFloat("_Depth", 0);
		}

		public override void RunStep (){
			if (simTextureManager == null) FindTextureManager();

			RenderTexture streamBedDepth = simTextureManager.CreateOutputTexture("StreamBed Depth", true);
			...
			// Update Heightmap:
			Graphics.Blit(m_clearTexture, streamBedDepth, m_depthClearMaterial);// Clear to near plane to allow ZTest GEqual
		...
		private GameObject CreateDepthCamera(){
			GameObject cameraObject = new GameObject("Surface Bed Depth Camera", typeof(Camera));
...
//			cameraObject.transform.Rotate(Vector3.right, 90);// Point it down. Terrain Height is farplane - (depth - offset)
			...
			depthCamera.useOcclusionCulling = false;// We need to render the objects that are farthest away from the camera.
